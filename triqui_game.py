...
class TriquiGame:
    def __init__(self, nombre1: str, nombre2: str, display, score_manager):
        self.nombre1 = nombre1 # Error en el nombramiento
        self.nombre2 = nombre2 # Error en el nombramiento
        self.display = display # Error en el nombramiento display
        self.score_manager = score_manager # Error en el nombramiento display
    ...
    def pedir_jugada(self) -> Tuple[int, int]:  # sin cast a int ni try/except
        while True:
            try:
                ...
                if self.tablero[fila - 1][col - 1] != " ":
                    print("⚠️  Esa casilla ya está ocupada.") # mensaje  poco explicito
                    continue
                return fila - 1, col - 1
            except Exception:
